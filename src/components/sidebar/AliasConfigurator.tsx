import { ButtonItem, ConfirmModal, DialogButton, Focusable, PanelSection, TextField, showModal } from "@decky/ui";
...
const AliasConfigurator: FC<{ alias: string; onChange: (alias: string) => void }> = ({ alias, onChange }) => {
  ...
  return (
    <Focusable style={{ display: "flex" }} flow-children="horizontal">
      ...
      <DialogButton onClick={search} disabled={searching || gameFound} style={{ maxWidth: "10em", display: "flex", justifyContent: "space-around" }}>
        {gameFound ? <MdCheck size={20} /> : <MdSearch size={20} />} Search
      </DialogButton>
    </Focusable>
  );
};
